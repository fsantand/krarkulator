...
function flipCoins(triggerAmount) {
  const flippedCoins = Array(krarkCopies.value * triggerAmount)
    .fill(0)
    .map(flipCoin)
...
function optimizeFlips({ wonFlips, lostFlips, choiceFlips }) {
  logAction({
    message: `Optimizing flips: ${wonFlips} won, ${lostFlips} lost, ${choiceFlips} to choose`,
  })
...
function castRitual() {
  ...
  logAction({
    message: `The cast cointains ${wonFlips.value} won flips and ${lostFlips.value} lost flips`,
  })
}
...
function castSpellWithStorm() {
  ...
  logAction({
    message: `The amount of copies of the spell is ${wonFlips.value + stormCount.value}`,
  })
  increaseStormCount()
}
